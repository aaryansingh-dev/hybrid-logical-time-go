...
func NewTrialEnded(at time.Time, customer string, clockID string) *TrialEnded {
	return &TrialEnded{at: at, customer: customer, clockID: clockID}
}
...
func (e *TrialEnded) ClockID() string {
	return e.clockID
}
...
func (e *TrialEnded) Execute(timeProvider clock.TimeProvider) []engine.Event {
	...
	return []engine.Event{
		NewInvoiceCreated(invoiceTime, e.customer, e.clockID),
	}
}
